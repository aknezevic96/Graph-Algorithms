...
class graph {
        ...
        for(edge &e : vertices[u].outgoing) {
          ...
          if(report[v].state == UNDISCOVERED) {
            ...
            q.push(v);
			  ...
			  if(report[v].dist > report[u].dist + 1) { //update npaths with better path
				  report[v].dist = report[u].dist + 1;
				  report[v].npaths = report[u].npaths;
			  }
			  
			  else if(report[v].dist == report[u].dist + 1) //another short path to vertex
				  report[v].npaths += report[u].npaths;		//increase number found
		  }
        }
      }
  ...
  public:
	...
	void reverse(vector<int> &v) { //helper function to reverse vector contents after push_backs
		int j = v.size() - 1;
    ...
    bool extract_path(const vector<vertex_label> & rpt, 
      ...
      for(int i = dest; i >= -1; i = rpt[i].pred) {
		  if(vertices[i].incoming.size() == 0) { //path found
			  path.push_back(i);
			  break;
		  }
		  
		  if(rpt[i].pred == -1) { //no path found, return false
			  path.clear();
			  return false;
		  }
	  ...
	  reverse(path); //make vector in "forward order"
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) {
	  ...
	  for(int i = 0; i < vertices.size(); i++) { //find all srcs and set up each dist and pred
	  ...
	  for(int i = 0; i < top.size(); i++) {
		  ...
		  for(edge &e : vertices[x].outgoing) { //go through each edge coming out of x
			  y = e.vertex_id;
			  
			  if(rpt[y].dist < rpt[x].dist + e.weight) { //if new vertex dist < x's dist + edge's weight
				  rpt[y].dist = rpt[x].dist + e.weight; //update new vertex dist
				  rpt[y].pred = x;						//x is y's new pred
			  }
		  }
	  }
    ...
    bool dag_num_paths(vector<vertex_label> & rpt) {
	  ...
	  for(int i = 0; i < vertices.size(); i++) { //starting and ending vertices
		  rpt[i].pred = 0;
		  if(vertices[i].incoming.size() == 0 || vertices[i].outgoing.size() == 0) {
			  rpt[i].npaths = 1; //inputs and i/o
	  ...
	  for(int i = top.size() - 1; i >= 0; i--) {
		  ...
		  for (edge &e : vertices[u].incoming) {
              ...
              if(vertices[v].incoming.size() > 0)
                  rpt[v].pred += rpt[u].pred;
		  }
	  }
	  
	  for(int i = 0; i < top.size(); i++) {
		  u = top[i];
		  rpt[u].npaths = rpt[u].npaths * rpt[u].pred;
	  }
    ...
    bool valid_topo_order(const vector<int> & order) {
	  ...
	  vector<bool> visited = vector<bool>(num_nodes()); //keeps track of what has been visited
	  
	  for(int i = 0; i < visited.size(); i++) //defaults all to false
		  visited[i] = false;
	  ...
	  for(int i = 0; i < order.size(); i++) {
		  ...
		  if(size > 0) { // is source?
			  for(int j = 0; j < size; j++) { //if not, check that all predecessors have been visited
				  if(visited[vertices[u].incoming[j].vertex_id] == false)
	...
	void _enum_paths(int t, vector<std::string> &paths, std::string s) { //helper function
		s = id2name(t) + s; //current
		
		if(vertices[t].incoming.size() == 0) { //is source node?
			paths.push_back(s);
			return;
		}
		
		for(edge &e : vertices[t].incoming)
			_enum_paths(e.vertex_id, paths, " " + s);
		
		std::sort(paths.begin(), paths.end()); //sort for tests
	}
	 
    bool enum_paths(int target, vector<string> &paths) {
      paths.clear();
      if(has_cycle() || target < 0 || target >= num_nodes()) //checks bounds
        return false;
	  
	  _enum_paths(target, paths, ""); //starts at target and goes to source
